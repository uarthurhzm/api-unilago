    ...
    public function GetNotices(GetNoticesDTO $data)
    {
        ...
        $stmt = Database::conn()->prepare(
            "SELECT FIRST 50    
                MURAL.RECADO,
                MURAL.DATA,
                COALESCE(MURAL.PRIORIDADE, '1') AS PRIORIDADE,
                COALESCE(MURAL.ASSUNTO, 'AVISO') AS TITULO,
                MURAL_DOCS.URL_DOC,
            ...
            WHERE 
                MURAL.RECADO IS NOT NULL
                AND (MURAL.CD_MAT IS NULL OR MURAL.CD_MAT = :CD_MAT)
